...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path === "/api/invites/accept" && request.method === "POST") {
      ...
      if (!meta) {
        return json(
          {
            error:
              "Invalid or expired invite, or your account email does not match the invitation",
          },
          400
        );
      }
    ...
    if (inviteMatch && request.method === "POST") {
      ...
      if (!created) {
        return json(
          { error: "Only the owner can invite, or the email is invalid" },
          400
        );
      }
      return json({
    ...
    if (projectMatch) {
      ...
      if (request.method === "GET") {
        ...
        const role = (await isOwner(redis, user.userId, projectId))
          ? "owner"
          : "member";
        return json({
          project: {
