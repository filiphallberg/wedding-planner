...
const INDEX_KEY = 'event-planner-local-projects';
const projectKey = (id: string) => `event-planner-local-project:${id}`;
...
export function loadLocalProjectState(projectId: string): EventState | null {
  try {
    const raw = localStorage.getItem(projectKey(projectId));
    if (!raw) return null;
    return parseEventState(JSON.parse(raw));
  } catch {
    return null;
  }
}

export function saveLocalProjectState(projectId: string, state: EventState): void {
  localStorage.setItem(projectKey(projectId), JSON.stringify(state));
  const list = readIndex();
  const now = new Date().toISOString();
  const next = list.map((m) => (m.id === projectId ? { ...m, updatedAt: now } : m));
  writeIndex(next);
}
...
export function migrateLegacyLocalStorageIfNeeded(): void {
  ...
  try {
    const raw = localStorage.getItem('event-planner-state');
    if (!raw) return;
    const state = parseEventState(JSON.parse(raw));
    if (!state) return;
    const id = crypto.randomUUID();
    const now = new Date().toISOString();
    ...
    writeIndex([meta]);
    localStorage.removeItem('event-planner-state');
  } catch {
    /* ignore */
  }
}
