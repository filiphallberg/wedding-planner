...
import type { EventState, Guest } from '../state/types';

function getSeatsForTable(state: EventState, tableId: string, seatCount: number): (Guest | null)[] {
  const slots: (Guest | null)[] = Array.from({ length: seatCount }, () => null);
  for (const g of state.guests) {
    const a = state.assignments[g.id];
    if (a && a.tableId === tableId && a.seatIndex >= 0 && a.seatIndex < seatCount) {
      slots[a.seatIndex] = g;
    }
  }
  return slots;
...
export function downloadSeatingPdf(state: EventState): void {
  ...
  writeLines(['Event seating'], 18, 'bold');
  writeBlock(`Generated ${dateStr}`, 10, 'normal');

  const tablesSorted = [...state.tables].sort((a, b) => compareStringsNatural(a.label, b.label));
  ...
  doc.save(`event-seating-${safeDate}.pdf`);
}
